...
class KillAura : Module() {
    ...
    // AutoBlock
    private val autoBlockValue = ListValue("AutoBlock", arrayOf("Range", "Fake", "Off"), "Off")
    ...
    private val autoBlockPacketValue =
        ListValue("AutoBlockPacket", arrayOf("AfterTick", "AfterAttack", "Vanilla", "Verus", "HytPit"), "AfterTick")
    ...
    override fun onDisable() {
        ...
        if (verusBlocking && !blockingStatus && !mc.thePlayer.isBlocking()) {
            verusBlocking = false
            if (autoBlockPacketValue.get() == "verus") {
    ...
    fun onMotion(event: MotionEvent) {
        ...
        if (!event.isPre() && autoBlockPacketValue.get() == "HytPit") {
            block()
            if (target == null) {
                unblock()
            }
        ...
        if (event.isPre()) {
            if (target != null && autoBlockPacketValue.get() == "HytPit") {
                val playerDistance =
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (verusBlocking
            ...
            && autoBlockPacketValue.get() == "Verus") {
            event.cancelEvent()
        }
    ...
    private fun runAttack() {
        ...
        if (!hitable || failHit) {
            ...
            if(autoBlockPacketValue.get() == "HytPit"){
    ...
    private fun updateRotations(entity: Entity): Boolean {
        ...
        val predictSize =
            if (predictValue.get()) floatArrayOf(minPredictSize.get(), maxPredictSize.get()) else floatArrayOf(
                0.0F,
                0.0F
            )
        ...
        val boundingBox = when (boundingBoxModeValue.get()) {
            "Head" -> AxisAlignedBB(
                max(bb.minX, bb.minX + predict[0]),
                max(bb.minY, bb.minY + predictSize[1]),
                max(bb.minZ, bb.minZ + predict[2]),
                min(bb.maxX, bb.maxX + predict[0]),
                min(bb.maxY, bb.maxY + predictSize[1]),
                min(bb.maxZ, bb.maxZ + predict[2])
            )
    ...
    private fun startBlocking(interactEntity: Entity, interact: Boolean) {
        ...
        if (autoBlockValue.get().equals(
                "range",
                true
            ) && mc.thePlayer.getDistanceToEntityBox(interactEntity) > autoBlockRangeValue.get()
        )
            return
