...
@ModuleInfo(name = "AntiVoid", category = ModuleCategory.PLAYER, description = "")
class AntiVoid : Module() {
    private val modeValue = ListValue("Mode", arrayOf("Blink", "TPBack", "MotionFlag", "PacketFlag", "GroundSpoof", "OldHypixel", "BlocksMC", "OldCubecraft", "Packet"), "Blink")
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        when (modeValue.get().toLowerCase()) {
            ...
            "blink" -> {
                if (!blink) {
                    val collide = NewFall(mc.thePlayer.posX, mc.thePlayer.posY, mc.thePlayer.posZ, 0.0, 0.0, 0.0, 0F, 0F, 0F, 0F).findCollision(60)
